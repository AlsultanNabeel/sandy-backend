...
def get_persona_directives(
    ...
    **The `sandy_memories` read below feeds three of the six blocks, and nothing
    ...
    if message:
        # البحث بالكلمة — **كطبقة تانية مش وحيدة** جنب البحث بالمعنى.
        #
        ...
        if found:
            blocks.append(found)
        # الفهرسة (للبحث بالمعنى) بتنشغّل ع الخلفية، مش جوّا القراءة.
        #
        ...
        #
        # و**بعد** البحث مش قبله: التنين بيقروا نفس القوائم من نفس الكاش، ولمّا
        # الفهرسة كانت تنطلق أول، الاتنين بيلاقوا الكاش فاضي بنفس اللحظة وبيقروا
        # القوائم الخمس مرّتين — سبع رحلات زيادة بأول دور بعد أي تعديل.
