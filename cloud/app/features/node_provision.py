...
PART_CATALOGUE: Dict[str, Dict[str, Any]] = {
    ...
    "ir": {
        ...
        # **بلا `meta` بالمرّة، وهاد مقصود ومهم.**
        #
        # أزرار جهاز الأشعة ما بينكتبوا هون — بينتعلّموا: المالك بيحطّ اللوح
        # بوضع التعلّم، بيكبس ريموته، والكود بينحفظ باسم بيختاره هو. قائمة
        # ثابتة هون كانت رح تفترض إنّ كل بيت عنده نفس الأجهزة، وهي الفرضية
        # اللي هالميزة كلها موجودة عشان تكسرها.
        #
        # والأهم: `_refresh_from_catalogue` بتدمج ميتا الكتالوج فوق الموجودة
        # بكل نبضة. فلو كتبنا `"meta": {"buttons": {}}` — حتى فاضية — كانت
        # بتدهس كل زر اتعلّمه المالك، كل خمس ثواني، للأبد. الميتا الغائبة هي
        # الطريقة الوحيدة تقول «هاد الحقل ملك المالك مش ملكي».
    },

    # ── عقدة الغرفة ──────────────────────────────────────────────────────────
    #
    # ببادئة `room/` لنفس سبب `cam/`: لوح تالت تحت نفس معرّف الوحدة، وضوء
    # الغرفة ما بيقعد جنب رقبة الروبوت بقائمة وحدة.
    "room/light": {
        "name": "room_light", "label": "ضوء الغرفة", "control_type": "switch",
        # مفتاح مش شريط تعتيم: السيرفو بيكبس القلّاب كبسة ميكانيكية. اللوح
        # بيقبل رقم ويعتبر أي إشي فوق الصفر تشغيلًا، بس شريط بيتحرّك بين عشرة
        # وتسعين وما بيغيّر إشي بيكذب على المستخدم مرّتين بالدقيقة.
    },
    "room/music": {
        "name": "room_music", "label": "موسيقى الغرفة", "control_type": "enum",
        # القيم اللي بيفهمها المشغّل حرفيًّا (`handleMusic` بـ room-node.ino).
        # `play` ناقصة بالقصد: التشغيل بده مجلد ومقطع (`play:1:3`)، وهاد مش
        # خيار من قائمة — بينضاف لمّا يصير في واجهة تختار المقطع.
        "meta": {"values": ["stop", "pause", "resume", "next", "prev"]},
    },

    "cam/quality": {
        "name": "cam_quality", "label": "وضوح الصورة", "control_type": "enum",
        # **ثلاثة أسماء، مش رقم مقلوب.**
        #
        # الرقم الحقيقي عند المستشعر من عشرة لثلاثة وستّين، **والأصغر أوضح** —
        # يعني شريط بتسحبه لليمين بيخلّي الصورة أسوأ. ما في مستخدم بيتوقّع هيك،
        # ولو عرضناه كما هو بيصير كل استعمال تخمينًا.
        #
        # اللوح بيترجم الاسم لرقمه. والمقياس الحقيقي بيضلّ موجود لمين بدّه
        # يضبطه بدقّة — بس مش هو الواجهة.
