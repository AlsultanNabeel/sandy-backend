...
static void publishStatus() {
  ...
  // **`outputs` هي اللي بتخلّي الغرفة تظهر بالتطبيق.**
  //
  // الجهاز بينوجد لأنّ لوح أعلن عنه، مش لأنّ حدا كتب اسمه بشاشة إضافة. بلا
  // هالحقل العقدة بتشتغل وبتضل غير موجودة: بتتصل، وبتنفّذ الأوامر، وشاشة
  // الإضافة ما بتقدر تعرض ضوءًا لأنّ ما حدا أعلن عن ضوء — وبيصير المالك لازم
  // يوصف عتاده لتطبيقه بإيده.
  //
  // الأسماء مجرّدة من البادئة؛ الخادم بيحطّ `room/` قدامها متل ما بيعمل مع
  // الكاميرا. وهي **نفس أسماء جدول `DEVICES`** فوق: سطر جديد هناك بده سطر
  // هون، وإلا الجهاز بيشتغل وما بيظهر.
  char buf[360];
  snprintf(buf, sizeof(buf),
           "{\"uptime_s\":%lu,\"rssi\":%d,\"heap\":%u,\"light\":\"%s\","
           "\"ip\":\"%s\",\"board\":\"%s\",\"fw\":\"%s\","
           // `kind` لازم يكون من قائمة الخادم المعروفة (`KNOWN_CAPABILITIES`
           // بـ node_store) — نوع خارجها بينرفض **بصمت**، والعرض إنّ الجهاز
           // ما بيظهر بالتطبيق ولا رسالة خطأ بأي مكان.
