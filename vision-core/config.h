...
// **دقّة متوسطة عند الإقلاع — والتكبير لحظة الالتقاط.**
//
// جرّبت أبدأ بالأكبر (`UXGA`) عشان يكون المخزن واسع من البداية، وكان قرارًا
// غلطًا بثمن كبير: مشغّل المستشعر بيحجز مخازن نقل بالذاكرة **الداخلية** بحجم
// متناسب مع الدقّة. فع `UXGA` أكلها، وما ضلّ كتلة متّصلة كافية للاتصال
// المشفّر مع الوسيط:
//
//     free=47532 largest=10228 → SSL - Memory allocation failed
//
// يعني اللوح صار يصوّر بأعلى دقّة **وما بيقدر يوصل الخادم** — وهاد أسوأ من
// دقّة أقلّ بمراحل. الكاميرا اللي ما بتوصل ما إلها قيمة مهما كانت صورتها.
//
// فبنقلع بدقّة متوسطة، والتكبير بيصير وقت الالتقاط لحظة ما ينطلب: المخزن
// الكبير بيعيش ثواني بدل ما يحجز الذاكرة للأبد.
#define CAMERA_DEFAULT_FRAME_SIZE FRAMESIZE_VGA   // 640x480 — والتكبير وقت الالتقاط
...
// **خمس مئة بايت خام → حوالي سبع مئة بعد الترميز.**
//
// كانت ألف وأربعة وعشرين، يعني الرسالة بتطلع حوالي ألف وأربعمية بايت. والقياس
// من الحقل قال إشيًا واضحًا: نبضة الكاميرا (خمس مئة بايت) بتوصل، وقطعة الصورة
// (ألف وأربعمية) **ما بتوصل ولا مرّة** — نفس اللوح، نفس الاتصال، نفس اللحظة،
// والاتصال ما انقطع. لمّا يكون كل إشي متطابق إلا الحجم، الحجم هو المتغيّر.
//
// الثمن: قطع أكتر لنفس الصورة. وهاد مقبول — صورة بتوصل بثانيتين أحسن من صورة
// ما بتوصل، والفاصل بينهن بيمنع الانفجار أصلًا.
#define SNAPSHOT_CHUNK_RAW_BYTES    512
#define MQTT_BUFFER_SIZE            2048            // يكفي لـ chunk + JSON overhead
// فاصل بين القطع — **مش تجميلة، هو الفرق بين وصول الصورة وضياعها.**
//
// كان خمسة، يعني تسع رسائل بأربعين ميلي ثانية. وهاد مش «إرسال»، هاد انفجار:
// أربعتعش كيلوبايت مشفّرة بلحظة وحدة على لوح صغير وخطّ منزلي.
//
// والدليل من الحقل، من الطرفين لنفس الطلب: خمس قطع وصلت وانعرضت، وتسع قطع
// نُشرت كلها بنجاح ووصل منها **صفر**. اللوح ما اشتكى — النشر بجودة صفر ما إله
// إقرار، فهو بيكتب ع المقبس وبيمشي، والضياع بيصير بعده وما حدا بيعرف.
//
// وسبعين ميلي ثانية بتوصّل صورة كاملة بأقلّ من ثانية — أبطأ من انفجار بيضيع،
// وأسرع بما لا يُقاس من صورة ما بتوصل.
#define SNAPSHOT_INTER_CHUNK_DELAY_MS 70

// وتيرة البثّ البعيد — إطار كل تلت ثانية.
//
// أسرع من هيك بيشغّل اللوح كله بالتشفير والإرسال، فبيبطّئ الأوامر ونبضة
// الحالة — يعني الروبوت بيصير بطيء عشان الصورة تصير أنعم. والفرق بين تلاتة
// وستّة إطارات بالثانية ما بيستاهل لوحًا مشغولًا.
#define CAM_REMOTE_STREAM_INTERVAL_MS 330

// أقصى مدّة للبثّ البعيد قبل ما يوقف لحاله — خمس دقايق.
//
// البثّ المحلي عنده مهلة من زمان، والبعيد انضاف بلاها. فلو المستخدم سكّر
// التطبيق بلا ما يوقّفه، اللوح بيضلّ يصوّر ويرفع **للأبد**: بيسخن، وبياكل
// بيانات، وبيحجز الحلقة — فبيصير بطيء بالردّ ع الأوامر، وبتفشل الترقية عن بعد
// لأنه ما بيلحق يردّ عليها.
//
