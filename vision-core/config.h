...
// نسخة التطوير: ترقية ع الشبكة المحلية + مرآة السجل ع التلنت. مطفية بالنسخة
// اللي بتنباع؛ للتطوير بتنحط `#define SANDY_DEV 1` بـ secrets.h.
#ifndef SANDY_DEV
  #define SANDY_DEV 0
#endif
...
// **وهي كمان الحدّ الأعلى.** مخزن الإطار بيتحجز مرّة وحدة وقت التشغيل ع قدّ
// هالدقّة، ودقّة أكبر بعدين ما بتكبّره: الصورة بتطلع مقطوعة أو الالتقاط بيفشل
// من أصله. كان في منطق «بنكبّر وقت الالتقاط وبنرجّع»، ومبني ع افتراض إنّ المشغّل
// بيعيد الحجز — وهو ما بيعيد. فالدقّة الأعلى ما عادت خيارًا بيقبله اللوح.
...
// حجم مخزن رسائل الوسيط. الصورة ما عادت بتمرق بالوسيط (بترتفع بطلب للخادم)،
// فالمخزن بس للأوامر والحالة — أكبرها «الحالة الكاملة» بحوالي ألف بايت.
...
// مفتاح البث المحلي. **فاضي هون معناه «ولّد واحد عشوائي كل إقلاع»، مش «بلا
// حماية».** كان الفاضي يعني خادمًا مفتوحًا لأي جهاز ع شبكة البيت — والبث بكاميرا
// بغرفة. المفتاح العشوائي بيوصل التطبيق عن طريق الخادم وبس (شوف النبضة).
#ifndef CAM_HTTP_TOKEN
  #define CAM_HTTP_TOKEN ""
#endif
#define CAM_STREAM_IDLE_TIMEOUT_MS 120000  // بث بلا متفرّج → يوقف لحاله
// أطول جلسة مشاهدة محلية. نفس سقف البث البعيد: ما في بث بيضلّ شغّال لأنّ حدا
// نسي صفحة مفتوحة.
#define CAM_LOCAL_STREAM_MAX_MS   (5UL * 60UL * 1000UL)
// أقلّ فاصل بين طلبات التصوير والبث — حماية للفلاش من الحرارة وللوح من الإغراق.
